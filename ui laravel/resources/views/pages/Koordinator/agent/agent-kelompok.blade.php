...
@section('content')
        ...
        window.sendMessage = function() {
            ...
            fetch(route, {
                ...
                .then(data => {
                    ...
                    // Update conversation history dan sidebar - DENGAN AWAIT & ERROR HANDLING
                    (async function() {
                        try {
                            console.log('[history] Reloading history after new message...');
                            conversationHistory = await loadConversationHistoryFromMongoDB();
                            console.log('[history] History reloaded with', conversationHistory.length, 'messages');
                            const searchValue = document.getElementById('historySearchInput')?.value || '';
                            renderHistoryList(searchValue);
                            console.log('[history] ✅ Sidebar updated with new message');
                        } catch (error) {
                            console.error('[history] Error updating history sidebar:', error);
                        }
                    })();
    ...
    <script>
        // ============== CHAT HISTORY SIDEBAR ==============
    <script>
        // ============== CHAT HISTORY SIDEBAR WITH MONGODB INTEGRATION ==============
        
        // IMPORTANT: Ensure user is authenticated
        ...
        // Log auth status
        console.log('[history] Auth Status - User ID:', currentUserId, '- Authenticated:', currentUserId !== null);

        /**
         * Generate session title dari first user message (maksimal 50 chars)
         */
        function generateSessionTitle(messages) {
            if (!messages || messages.length === 0) return 'Percakapan baru';
        ...
        /**
         * Get group name berdasarkan timestamp (Hari Ini, Kemarin, Minggu Ini, Lebih Lama)
         */
        function getGroupName(timestamp) {
            ...
            if (msgDay === todayDay) return 'Hari Ini';
            if (msgDay === yesterdayDay) return 'Kemarin';
        ...
        /**
         * Format time display untuk history list
         */
        function formatTimeDisplay(timestamp) {
            ...
            const msgDay = date.toDateString();
            const todayDay = today.toDateString();
            const yesterdayDay = yesterday.toDateString();
            
            if (msgDay === todayDay || msgDay === yesterdayDay) {
        ...
        /**
         * Load conversation history dari MongoDB via API
         * Dengan comprehensive error handling & logging
         */
        async function loadConversationHistoryFromMongoDB() {
            ...
            try {
                ...
                if (currentUserId === null || currentUserId === undefined) {
                    console.warn('[history] User not authenticated! Cannot load history.');
                    return [];
                }

                const apiUrl = `http://localhost:8002/long-term-history/${currentUserId}?days=30&limit=200`;
                ...
                if (!response.ok) {
                    console.error('[history] API error response:', response.status, response.statusText);
                    ...
                    try {
                        ...
                        console.error('[history] Could not parse error response body');
                    }
                ...
                if (data.success && Array.isArray(data.history)) {
                    ...
                    if (data.history.length > 0) {
                        console.log('[history] First message:', data.history[0]);
                        console.log('[history] Last message:', data.history[data.history.length - 1]);
                    }
                    
                    isHistoryLoading = false;
                    return data.history;
                } else {
                    console.warn('[history] Invalid response format:', data);
                    isHistoryLoading = false;
                    return [];
                }
            } catch (error) {
                console.error('[history] ❌ Exception loading conversation history:', error);
                ...
                if (error instanceof TypeError) {
                    console.error('[history] Network error - API might not be running on http://localhost:8002');
                }
        ...
        /**
         * Group messages by session (jika gap > 30 menit = session baru)
         */
        function groupMessagesBySessions(messages) {
            ...
            messages.forEach(msg => {
                const msgTime = new Date(msg.timestamp || msg._id).getTime();
            ...
            console.log(`[history] Grouped ${messages.length} messages into ${sessions.length} sessions`);
            return sessions;
        }

        /**
         * Render history list dengan grouped sessions
         */
        function renderHistoryList(filterText = '') {
            console.log('[history] Rendering history list with filter:', filterText);
            
            const listEl = document.getElementById('historyList');
            if (!listEl) {
                console.error('[history] ❌ historyList element not found!');
                return;
            }
            ...
            console.log('[history] Sessions to render:', sessions.length);
            
            if (sessions.length === 0) {
                console.log('[history] No sessions found, showing empty state');
                listEl.innerHTML = `
            ...
            const sessionItems = sessions.map((session, idx) => {
                const lastMsg = session[session.length - 1];
                const title = generateSessionTitle(session);
                const timestamp = lastMsg.timestamp || lastMsg._id;
                const groupName = getGroupName(timestamp);
                ...
                groups[groupName].forEach(session => {
                    ...
                    item.innerHTML = `
                        ...
                        <div class="history-item-text">
                            <p class="history-item-title" title="${session.title}">${session.title}</p>
        ...
        /**
         * Load dan display history session yang dipilih
         */
        function loadHistorySession(session) {
            console.log('[history] Loading session:', session.id, 'dengan', session.messages.length, 'messages');
            
            // Set current session
            currentSessionId = session.id;
            conversationHistory = session.messages;

            // Clear dan render chat area dengan history
            const chatBox = document.getElementById('chatBox');
            if (chatBox) {
                chatBox.innerHTML = '';
                
                // Display semua messages dari session
        ...
        /**
         * Initialize history sidebar dengan data dari MongoDB
         * Dengan comprehensive error handling
         */
        async function initHistorySidebar() {
            console.log('[history] ==================== INITIALIZING HISTORY SIDEBAR ====================');
            console.log('[history] User ID:', currentUserId);
            console.log('[history] Current URL:', window.location.href);
            console.log('[history] Page loaded at:', new Date().toISOString());
            
            // Check if user is authenticated
            if (currentUserId === null || currentUserId === undefined) {
                console.warn('[history] ⚠️ User not authenticated! History will not load.');
                console.warn('[history] Please login first.');
                const listEl = document.getElementById('historyList');
                if (listEl) {
                    listEl.innerHTML = `
                        <div class="history-empty">
                            <p style="color: red; font-weight: bold;">⚠️ Silakan login terlebih dahulu</p>
                        </div>`;
                }
                return;
            }
            ...
            try {
                // Load conversation history dari MongoDB
                console.log('[history] Starting to load conversation history...');
                conversationHistory = await loadConversationHistoryFromMongoDB();
                
                console.log('[history] ✅ History loaded successfully');
                console.log('[history] Total messages loaded:', conversationHistory.length);
                
                // Render history list
                console.log('[history] Rendering history list...');
                renderHistoryList('');
                console.log('[history] ✅ History list rendered');

                // Setup search
                const searchInput = document.getElementById('historySearchInput');
                if (searchInput) {
                    console.log('[history] Setting up search input listener');
                    searchInput.addEventListener('input', (e) => {
                        console.log('[history] Search filter changed:', e.target.value);
                        renderHistoryList(e.target.value);
                    });
                } else {
                    console.warn('[history] Search input element not found');
                }

                // Setup new session button - WITH PROPER ERROR HANDLING & DEBUGGING
                const newSessionBtn = document.getElementById('newSessionBtn');
                console.log('[history] Finding new session button...');
                console.log('[history] Button element:', newSessionBtn);
                
                if (newSessionBtn) {
                    console.log('[history] ✅ New session button found, attaching listener');
                    
                    const handleNewSession = () => {
                        console.log('[history] ==================== NEW SESSION CLICKED ====================');
                        currentSessionId = null;
                        conversationHistory = [];
                        console.log('[history] Reset: currentSessionId = null, conversationHistory = []');
                        
                        // Clear chat box
                        const chatBox = document.getElementById('chatBox');
                        if (chatBox) {
                            chatBox.innerHTML = '';
                            console.log('[history] Chat box cleared');
                        } else {
                            console.warn('[history] Chat box not found');
                        }
                        
                        // Show landing view, hide chat view
                        const landingView = document.getElementById('landingView');
                        const chatView = document.getElementById('chatView');
                        
                        if (landingView) {
                            landingView.style.display = 'flex';
                            console.log('[history] Landing view shown');
                        } else {
                            console.warn('[history] Landing view not found');
                        }
                        
                        if (chatView) {
                            chatView.style.display = 'none';
                            console.log('[history] Chat view hidden');
                        } else {
                            console.warn('[history] Chat view not found');
                        }
                        ...
                        if (input) {
                            input.value = '';
                            input.focus();
                            console.log('[history] Input cleared & focused');
                        } else {
                            console.warn('[history] Input not found');
                        }
                        ...
                        if (searchInput) {
                            searchInput.value = '';
                            console.log('[history] Search input cleared');
                        }
                        
                        // Re-render sidebar (should show empty state)
                        renderHistoryList('');
                        
                        console.log('[history] ✅ New session initialized');
                    };
                    
                    newSessionBtn.addEventListener('click', handleNewSession);
                    console.log('[history] ✅ New session button listener attached successfully');
                } else {
                    console.error('[history] ❌ CRITICAL: New session button not found in DOM!');
                    console.log('[history] Checking DOM:', {
                ...
                if (toggleBtn && wrap) {
                    console.log('[history] Setting up sidebar toggle');
                    toggleBtn.addEventListener('click', () => {
                        wrap.classList.toggle('sidebar-collapsed');
                    });
                } else {
                    console.warn('[history] Sidebar toggle or wrap element not found');
                }

                console.log('[history] ✅ ==================== HISTORY SIDEBAR INITIALIZED SUCCESSFULLY ====================');
            } catch (error) {
                console.error('[history] ❌ Fatal error initializing history sidebar:', error);
                ...
                if (listEl) {
                    listEl.innerHTML = `
                        <div class="history-empty">
                            <p style="color: red; font-weight: bold;">❌ Error loading history:</p>
        ...
        // Fallback: Also try immediate initialization
